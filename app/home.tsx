...
type AIMode = 'instant' | 'deep-thinking' | 'agent';
...
// All modal components are imported from components/HomeModals.tsx
...
const shazamCardStyles = StyleSheet.create({
  tapCard: { borderRadius: 20, padding: 24, alignItems: 'center', justifyContent: 'center', minHeight: 180, borderWidth: 1, borderColor: 'rgba(13,114,234,0.2)' },
  shazamCircle: { width: 90, height: 90, borderRadius: 45, backgroundColor: '#0D72EA', alignItems: 'center', justifyContent: 'center', marginBottom: 8 },
  resultCard: { borderRadius: 20, overflow: 'hidden', borderWidth: 1, borderColor: 'rgba(13,114,234,0.15)' },
  albumArt: { width: '100%', height: 180 },
  songInfo: { padding: 16 },
  saveBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, backgroundColor: '#0D72EA', borderRadius: 24, paddingVertical: 12, paddingHorizontal: 20 },
  playBtn: { position: 'absolute', bottom: 16, right: 16, width: 44, height: 44, borderRadius: 22, backgroundColor: '#FFF', alignItems: 'center', justifyContent: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.2, shadowRadius: 4, elevation: 4 },
});
...
function MentionPopup({ members, onSelect, onClose }: {
  ...
  return (
    <View style={[mentionStyles.container, { backgroundColor: isDark ? '#1C1C1E' : '#FFFFFF', borderColor: isDark ? 'rgba(255,255,255,0.14)' : 'rgba(0,0,0,0.1)' }]}>
      {members.slice(0, 5).map((member, index) => (
        <TouchableOpacity key={member.id} style={[mentionStyles.row, index > 0 && { borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.06)' }]} onPress={() => onSelect(member)} activeOpacity={0.7}>
          {member.profile_photo_url ? (
            <ExpoImage source={{ uri: member.profile_photo_url }} style={mentionStyles.avatar} contentFit="cover" />
          ) : (
            <View style={[mentionStyles.avatarFallback, { backgroundColor: member.accent_color || '#10A37F' }]}>
              <Text style={mentionStyles.avatarLetter}>{(member.username || '?').slice(0, 1).toUpperCase()}</Text>
            </View>
          )}
          <Text style={[mentionStyles.name, { color: colors.text }]}>@{member.username}</Text>
        </TouchableOpacity>
      ))}
    </View>
  );
