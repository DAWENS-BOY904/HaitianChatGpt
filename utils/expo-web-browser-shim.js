...
const SafeWebBrowser = {
  ...
  openBrowserAsync: function (url, options) {
    // On native, use expo-web-browser in-app browser if available.
    ...
    // On web, always open inside an iframe overlay, never leaving the app.
    try {
  ...
  openAuthSessionAsync: function (url, redirectUrl, options) {
    // On native, use expo-web-browser auth session if available.
    if (typeof document === 'undefined') {
      try {
    ...
    // On web, stay inside the app using the iframe overlay.
